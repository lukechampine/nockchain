    ...
    fn submit(self) -> Submission<Self::Output> {
            ...
            for (o, iter) in s.iters.iter().enumerate() {
                ...
                let traces = gpu
                    ...
                    .create_buffer_init(&wgpu::util::BufferInitDescriptor {
                        ...
                        contents: bytemuck::cast_slice(&iter.traces.0),
                ...
                iter_buffers.push((traces, ops, ops_offsets, buf_ids, ops_buf, subs, mul_splits));
            }
        ...
        while let Some(stage) = stages.pop() {
                ...
                stage.iters.into_iter().enumerate().zip(ib)
            {
                ...
                let final_offset = (i % chunks_per_buf) * poly_len; // * core::mem::size_of::<Melt>();
                ...
                for (o, split_chunk) in mul_splits.chunks(chunk_size / poly_len).enumerate() {
                    ...
                    for (buf_id, input) in [(None, &traces)].into_iter().chain(
                        ...
                        chunk_muls.push((
                            (offs.num_ops as usize) * poly_len,
                            input.clone(),
                            output.clone(),
                            ops_buf.clone(),
                            subs.clone(),
                    ...
                    if split_chunk.len() > 1 {
                        ...
                        while accum_ops > 2 {
                            ...
                            let offs = AccumUniform {
                                inp_a: 0,
                                inp_b: (poly_len * right_ops) as _,
                                num_elems: (poly_len * num_ops) as _,
                                out: 0 as _,
                                out_mask: 0,
                            };
                        ...
                        trace!("finalaccum {poly_len}");

                        let offs = AccumUniform {
                            inp_a: 0,
                            inp_b: poly_len as _,
                            num_elems: poly_len as _,
