...
// calc digest
pub fn tip5_calc_digest(sponge: &[Melt; 16]) -> [u64; 5] {
    let mut digest = [0u64; DIGEST_LENGTH];
    for i in 0..DIGEST_LENGTH {
        digest[i] = Belt::from(sponge[i]).0;
    }
    digest
}

// absorb complete input
pub fn tip5_absorb_input(mut input_to_absorb: &[Melt], mut sponge: &mut [Melt; 16], q: usize) {
    let mut cnt_q = q;
    loop {
...
fn hash_varlen(mut input_vec: Vec<Belt>) -> [u64; 5] {
    ...
    let input_vec = input_vec.into_iter().map(Melt::from).collect::<Vec<_>>();
...
pub fn hash_10_jet(context: &mut Context, subject: Noun) -> Result<Noun, JetErr> {
    ...
    let digest = hash_10(input_vec);

    Ok(vec_to_hoon_list(stack, &digest))
}

fn hash_10(input_vec: Vec<Belt>) -> [u64; 5] {
    // check input
    let (q, r) = tip5_calc_q_r(&input_vec);
    assert_eq!(q, 1);
    assert_eq!(r, 0);
...
fn hash_ten_cell(stack: &mut NockStack, ten_cell: Noun) -> Result<Noun, JetErr> {
    ...
    let leaf_belt = leaf.into_iter().map(|x| Belt(x)).collect();

    // list-to-tuple hash10
    let digest = hash_10(leaf_belt);
    Ok(digest_to_noundigest(stack, digest))
}
