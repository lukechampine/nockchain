...
use crate::utils::{
    belt_as_noun, bitslice_to_u128, fits_in_u128, hoon_list_to_arraybelt, hoon_list_to_vecbelt,
    hoon_list_to_vecnoun, vec_to_hoon_list, vecnoun_to_hoon_list,
};
...
pub fn hash_varlen_jet(context: &mut Context, subject: Noun) -> Result<Noun, JetErr> {
    ...
    let digest = tip5::hash::hash_varlen(input_vec);
...
pub fn digest_to_noundigest(stack: &mut NockStack, digest: [Belt; 5]) -> Noun {
...
pub fn hash_10_jet(context: &mut Context, subject: Noun) -> Result<Noun, JetErr> {
    let stack = &mut context.stack;
    let input = slot(subject, 6)?;
    let input_vec = hoon_list_to_arraybelt(input)?;

    let digest = tip5::hash::hash_10(input_vec);
...
pub fn hash_pairs(stack: &mut NockStack, lis_noun: Noun) -> Result<Noun, JetErr> {
    ...
    for i in 0..lent_lis / 2 {
        ...
        if (b + 1) == lent_lis {
            ...
            let b0 = hoon_list_to_arraybelt::<5>(lis[b])?;
            let b1 = hoon_list_to_arraybelt::<5>(lis[b + 1])?;
            let pair = array_concat::concat_arrays!(b0, b1);
            let digest = tip5::hash::hash_10(pair);
...
fn hash_ten_cell(stack: &mut NockStack, ten_cell: Noun) -> Result<Noun, JetErr> {
    ...
    let leaf_belt = leaf.into_iter().map(Belt).collect::<Vec<_>>();
    let Ok(leaf_belt) = leaf_belt.try_into() else {
        return Err(BAIL_FAIL);
    };

    // list-to-tuple hash10
    let digest = tip5::hash::hash_10(leaf_belt);
