...
use zkvm_jetpack::jets::bp_jets::init_bpoly_sam;
use zkvm_jetpack::jets::fp_jets::{coseword_sam, init_fpoly_sam};
...
pub fn generate_proof(context: &mut Context, subject: Noun) -> Result {
    ...
    let base_tables = {
        ...
        // TODO: this isn't strictly necessary, it just matches the subsequent
        // code that expects a table-dat rather than a table-mary
    ...
    let augmented_chals = {
        ...
        BPolySlice::try_from(init_bpoly_sam(stack, augmented_list)?)?
    };
    ...
    let (omicrons_bpoly, omicrons_fpoly) = {
        ...
        let bpoly = BPolySlice::try_from(init_bpoly_sam(stack, omicrons)?)?;
        let fpoly = init_fpoly_sam(stack, lifted_omicrons)?;
        (bpoly, fpoly)
    };
