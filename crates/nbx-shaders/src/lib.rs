...
use wgpu::{Device, ShaderModuleDescriptor, ShaderModule, Label, ShaderSource};

#[cfg(not(feature = "comptime"))]
macro_rules! shader_path {
    ($shader:literal) => {
        concat!(env!("CARGO_MANIFEST_DIR"), "/../nbx-shaders-build/shaders-out/", $shader, ".spv")
    }
}
...
static SHADERS: [(&str, LazyLock<Cow<'static, [u32]>>); 1] = [
