    ...
    let kernel_f = async |checkpoint| {
        let kernel: Kernel<SaveableCheckpoint> = match cli.stack_size {
            NockStackSize::Normal => {
                Kernel::load_with_hot_state(jam, checkpoint, hot_state, cli.trace_opts).await?
            }
            NockStackSize::Big => {
                Kernel::load_with_hot_state_big(jam, checkpoint, hot_state, cli.trace_opts.into()).await?
            }
            NockStackSize::Huge => {
                Kernel::load_with_hot_state_huge(jam, checkpoint, hot_state, cli.trace_opts.into()).await?
            }
        };
