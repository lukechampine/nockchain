...
cat txnotes_$ADDR.txt | grep -a "\(assets\|signers\|name\)" > txfiltered
echo "[ $(cat txfiltered | sed 's/- name: \(.*\)/{ "name": "\1", /g' | sed 's/\.//g' | sed 's/- assets: \(.*\)/"assets": \1,/g' | sed 's/- signers: \(.*\)/"signers": "\1" },/g') ]" | sed 's/\(.*\), ]/\1 ]/g' > tx.json

jq --argjson threshold "$AMT" --argjson skip "$SKIP" '
  (
    .[$skip:]
    | reduce .[] as $i (
        {prev:0,sum:0,items:[]};
...
exec "$WALLET" simple-spend --names "$NAMES" --recipients "$RECIPIENTS" --gifts "$ASSETS" --fee 0 # | grep -a "saving draft to"
